...
class Node():
    ...
    def __init__(self):
        ...
        self.max_vel = 1.5 
        self.max_acc = 1.1
        self.formation = 2  # 0 = line | 1 = column | 2 = triangle | -1 = consensus OFF

        self.flock = [Boid(x, self.circle, self.max_vel, self.max_acc, self.num_of_robots, self.formation) for x in range(self.num_of_robots)]
        self.pubs = [rospy.Publisher('/robot_{}/cmd_vel'.format(x), Twist, queue_size=1) for x in range(self.num_of_robots)]
        self.pub_data = []

        self.target = np.array([3, 4])
    ...
    def goto(self):
        ...
        center_to_target  = self.target - self.flock[0].get_position()
